...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        ...
        if self.visual._get_name() == 'VisionTransformer':
            output = output[:, 0, :]
        return output

    def encode_image_map(self, image):
        output = self.visual(image.type(self.dtype))
        return output

    def clip_feature_surgery(self, image_features, text_features, redundant_feats=None, t=2):
        ...
        if redundant_feats != None:
            ...
            b, n_t, n_i, c = image_features.shape[0], text_features.shape[0], image_features.shape[1], \
