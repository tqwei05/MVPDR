...
class PlantVillage(DatasetBase):
    ...
    def __init__(self, root, num_shots):
        ...
        self.image_dir = os.path.join(self.dataset_dir, 'raw/color')
        # self.test_dir = os.path.join(self.dataset_dir, 'SVM/test')
        ...
        self.origin_classes = sorted(os.listdir(os.path.join(self.dataset_dir, "raw/color")))

        self.classes = ["apple scab", "apple black rot", "apple cedar rust", "healthy apple leaf", "healthy blueberry leaf",
                        "cherry powdery mildew", "healthy cherry leaf", "corn cercospora leaf spot", "corn common rust",
                        "corn northern leaf blight", "healthy corn leaf", "grape black rot ", "grape black measles",
                        "grape isariopsis leaf spot", "healthy grape leaf", "citrus greening", "peach bacterial spot", "healthy peach leaf",
                        "bell pepper bacterial spot", "healthy bell pepper leaf", "potato early blight",
                        "potato late blight", "healthy potato leaf", "healthy raspberry leaf", "healthy soybean leaf", "squash powdery mildew",
                        "strawberry leaf scorch", "healthy strawberry leaf", "tomato acterial spot", "tomato early blight",
                        "tomato late blight", "tomato leaf mold", "tomato septoria leaf spot", "tomato spider-mites",
                        "tomato target spot", "tomato yellow leaf curl virus", "tomato mosaic virus", "healthy tomato leaf"]
        di = dict(zip(self.origin_classes, self.classes))
        ...
        # with open(os.path.join(self.dataset_dir, "SVM/train_mapping.txt"), "r") as f:
        #     for line in f.readlines():
        #         str_lst = line.split("\t", 1)
        #         self.train_image_names_list.append(str_lst[-1][:-1])
        #
        # with open(os.path.join(self.dataset_dir, "SVM/test_mapping.txt"), "r") as f:
        #     for line in f.readlines():
        #         str_lst = line.split("\t", 1)
        #         self.test_image_names_list.append(str_lst[-1][:-1])



        # get all the image names and corresponding labels
        for i in range(len(self.classes)):
            c_root = os.path.join(self.image_dir, self.origin_classes[i])
            image_names = os.listdir(c_root)

            num = len(image_names)
            num_trainval = math.ceil(num * self.train_percent)
            trainval_indexes = random.sample(range(num), num_trainval)

            midpoint = math.ceil(len(trainval_indexes))
            ...
            for j in image_names:
                self.image_names_list.append(os.path.join(self.origin_classes[i], j))
    ...
    @staticmethod
    def read_split_txt(path, classes):
        def _convert(items):
            out = []
            for impath, label, classname in items:
                item = Datum(impath=impath, label=int(label), classname=classname)
